...
import { TeamBadge, PlayerPortrait, CoachPortrait, CompetitionEmblem, RatingPill } from './components/VisualIdentity.jsx';
...
function TeamMark({ team, size = 38, className = '' }) { return <TeamBadge team={team} size={size} className={className}/>; }
...
const MARKET_ROUTE_TO_TAB = { summary:'Offseason Summary', trades:'Trades', transfers:'Transfers', draft:'Draft', rights:'Rights', 'free-agency':'Free Agency', retirements:'Retirements', legacy:'Hall of Fame', spawn:'Spawn', coaches:'Coaches' };
...
export default function App() {
  ...
  const toggleFollowPlayer = (playerId) => {
    const followed = new Set(universe.followedPlayerIds ?? []);
    if (followed.has(playerId)) followed.delete(playerId);
    else followed.add(playerId);
    universe.followedPlayerIds = [...followed];
    commitUniverse({ ...universe });
  };
  ...
  return <div className="app-shell">
    <header className="masthead"><div className="brand-lockup"><span className="brand-ball">🏀</span><div><div className="kicker light">The Global Five presents</div><h1>Basketball World Chronicle</h1><p>World basketball · careers · dynasties · history</p></div></div><div className="masthead-actions"><div className="season-label"><strong>{universe.year}</strong><span>Week {universe.week} · {universe.phase}</span></div><button className="header-button" onClick={goHome}>⌂ Home</button><button className="header-button" onClick={() => void persistUniverse()}>💾 Save</button><span className={`save-status ${saveStatus.startsWith('Save failed') || saveStatus.includes('error') ? 'error' : ''}`}>{saveStatus}</span></div></header>
    ...
    <nav className="mobile-bottom-nav" aria-label="Mobile navigation">
      {[['World','⌂'],['The Global Five','★'],['Tournaments','🏆'],['Teams','◈'],['Players','♟'],['Market','⇄']].map(([item,icon])=><button key={item} className={view===item?'active':''} onClick={()=>navigate(item)}><span>{icon}</span><small>{item==='The Global Five'?'Magazine':item}</small></button>)}
    </nav>
  </div>;
}
...
function GameShowcaseView({ game, index, total, onCheckpoint, onSkip, onNext, onPlayer }) {
  ...
  return <div className="showcase-page"><div className="showcase-meta"><div><div className="kicker">{game.competition} · {game.round}{game.competitionId==='nba'?` · Game ${game.gameNumber}`:''}</div><h2>Postseason showcase</h2><p>Game {index+1} of {total}. Watch minute by minute, stop after each period, or skip directly to the final box score.</p></div><div className="showcase-actions">{!final&&<><button className="button primary" disabled={playing} onClick={playNextPeriod}>{playing?'Simulating…':`Play ${game.periodLabels[nextPeriodIndex]}`}</button><button className="button" disabled={playing} onClick={skip}>Skip game</button></>}{final&&<button className="button primary" onClick={onNext}>{index+1<total?'Next game':'Continue to Year Review'}</button>}</div></div>
    ...
    {final&&<><section className="panel game-mvp premium-mvp"><div className="kicker">Game MVP</div><button onClick={()=>onPlayer(game.mvp.playerId)}><PlayerPortrait player={{...mvpRow,name:game.mvp.player,rarity:mvpRow?.rarity||'Rare',position:mvpRow?.position||'—'}} size={76}/><div><strong>{game.mvp.player}</strong><span>{game.mvp.team}</span></div></button><div className="mvp-statline"><b>{game.mvp.points} PTS</b><span>{game.mvp.rebounds} REB ({game.mvp.orpg} O / {game.mvp.drpg} D)</span><span>{game.mvp.assists} AST</span><span>{game.mvp.steals} STL</span><span>{game.mvp.blocks} BLK</span></div></section><div className="showcase-box-grid"><GameBox team={game.teamA} score={game.scoreA} rows={game.boxA} onPlayer={onPlayer}/><GameBox team={game.teamB} score={game.scoreB} rows={game.boxB} onPlayer={onPlayer}/></div></>}
  </div>;
}
...
function DetailRouter({ detail, universe, teamById, playerById, coachById, onBack, onTeam, onPlayer, onCoach, onCompetition, onToggleFollow }) {
  if (detail.type === 'team') {
  ...
  if (detail.type === 'player') { const player = playerById.get(detail.id); return player ? <PlayerPage player={player} universe={universe} onBack={onBack} onTeam={onTeam} onCompetition={onCompetition} onToggleFollow={onToggleFollow} /> : <NotFoundPage title="Player not found" />; }
...
function WorldView({ universe, setView, setMarketTab, onTeam, onPlayer }) {
  ...
  return <>
    <section className="world-hero modern-hero"><div className="world-hero-copy"><div className="kicker light">The Global Five · {universe.year}</div><h2>The living basketball world</h2><p>Follow careers from college and club systems to the NBA, EuroLeague and international basketball. Every title, draft, transfer and great season becomes part of the permanent archive.</p><div className="story-actions"><button className="button primary" onClick={() => setView('Tournaments')}>Explore competitions</button><button className="button ghost-light" onClick={() => goToRoute('market/free-agency')}>Open market</button></div></div><div className="world-hero-stars">{leaders.slice(0,3).map((player,index)=><button key={player.id} className={`hero-player-card hero-rank-${index+1}`} onClick={()=>onPlayer(player)}><PlayerPortrait player={player} size={84}/><div><Rarity value={player.rarity}/><strong>{player.name}</strong><span>{player.teamName} · {player.current} OVR</span></div></button>)}</div></section>
    <div className="metric-grid world-metrics"><Metric label="NBA average" value={avg(nba).toFixed(1)} detail="Deepest talent pool" /><Metric label="EuroLeague average" value={avg(euro).toFixed(1)} detail="Top non-NBA clubs" /><Metric label="NCAA average" value={avg(ncaa).toFixed(1)} detail="Prospect level" /><Metric label="Free agents" value={universe.freeAgents.length} detail="Visible market pool" /></div>
    <section className="two-column section-gap"><section className="panel visual-panel"><SectionHeader title="Global team power" note="National teams excluded" /><div className="rank-list">{topTeams.map((team,index) => <button className="rank-row rank-button visual-rank" key={team.id} onClick={() => onTeam(team)}><span className="rank-number">{index+1}</span><TeamMark team={team} size={46}/><div className="grow"><strong><Flag value={team.country}/> {team.name}</strong><span>{team.competition} · raw {team.rawRating}</span></div><RatingPill value={team.rating}/></button>)}</div></section><section className="panel visual-panel"><SectionHeader title="Best players today" note="Open a full career" /><div className="player-mini-grid">{leaders.map((player) => <button className={`mini-player-card rarity-surface-${player.rarity.toLowerCase()}`} key={player.id} onClick={() => onPlayer(player)}><PlayerPortrait player={player} size={66}/><div><Rarity value={player.rarity}/><strong>{player.name}</strong><span><Flag value={player.nationality}/> {player.position} · {player.teamName}</span></div><RatingPill value={player.current}/></button>)}</div></section></section>
    <section className="panel section-gap"><SectionHeader title="League strength" note="Current roster, coaching and institutional context"/><div className="table-wrap"><table><thead><tr><th>#</th><th>Competition</th><th>Region</th><th>Teams</th><th>Average rating</th></tr></thead><tbody>{leagueStrengths.map((row,index)=><tr key={row.competition.id}><td>{index+1}</td><td><span className="table-identity"><CompetitionEmblem competition={row.competition} size={32}/><strong>{row.competition.name}</strong></span></td><td><Flag value={row.competition.region}/> {row.competition.region}</td><td>{row.teams}</td><td><strong>{row.rating.toFixed(1)}</strong></td></tr>)}</tbody></table></div></section>
  </>;
}
...
function CompetitionCards({ competitions, universe, onCompetition }) { return <div className="card-grid three competition-grid-modern">{competitions.map((competition) => { const teams = getCompetitionParticipants(universe, competition.id); const latest = universe.competitionHistory[competition.id]?.[0]; const active = isCompetitionActive(competition, universe.year); return <button className="competition-card competition-button modern-competition-card" key={competition.id} onClick={() => onCompetition(competition)}><div className="competition-visual-head"><CompetitionEmblem competition={competition} size={60}/><div><div className="kicker"><Flag value={competition.country || competition.region}/> {competition.country || competition.region} · {competition.kind}</div><h3>{competition.name}</h3></div></div><div className="competition-card-meta"><strong>{teams.length || latest?.standings?.length || 0} teams</strong><span className={active?'live-chip':''}>{active ? '● Active now' : competition.frequency ? `Every ${competition.frequency} years` : competition.detail}</span></div><div className="mini-badge-row">{teams.slice(0,6).map(team=><TeamMark team={team} size={28} key={team.id}/>)}</div><p>{latest ? `${latest.year}: ${latest.champion} · MVP ${latest.mvp.name}` : 'No completed edition yet.'}</p></button>; })}</div>; }
...
function TeamsView({ universe, onTeam }) {
  ...
  return <><div className="page-heading"><div><div className="kicker">Teams</div><h2>Region → country → team history</h2></div><div className="page-note">Visual team identities, live strength and permanent history.</div></div><div className="tabs">{regions.map((item) => <button key={item} className={region === item ? 'active' : ''} onClick={() => {setRegion(item);setCountry(unique(universe.teams.filter((team)=>team.region===item).map((team)=>team.country))[0]);setCompetition('All');}}><Flag value={item}/> {item}</button>)}</div><div className="tabs countries">{countries.map((item)=><button key={item} className={safeCountry===item?'active':''} onClick={()=>{setCountry(item);setCompetition('All');}}><Flag value={item}/> {item}</button>)}</div><div className="filters compact"><Filter label="Competition"><select value={competition} onChange={(e)=>setCompetition(e.target.value)}><option>All</option>{competitions.map((item)=><option key={item}>{item}</option>)}</select></Filter><Filter label="Search"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Team name"/></Filter></div><div className="card-grid four team-card-grid-modern">{shown.map((team)=><button className="team-card modern-team-card" key={team.id} onClick={()=>onTeam(team)} style={{'--team-color':team.color}}><div className="team-card-glow"/><div className="team-card-top"><TeamMark team={team} size={62}/><RatingPill value={team.rating}/></div><h3>{team.name}</h3><p><Flag value={team.country}/> {team.type==='NCAA'?team.secondaryCompetitions.at(-1):team.competition}</p><div className="tag-row"><span>{team.rosterIds.length} players</span><span>{team.honors.length} titles</span>{team.tier===2&&<span className="danger-tag">Tier 2</span>}{team.secondaryCompetitionIds.includes('euroleague')&&<span className="blue-tag">EuroLeague</span>}</div></button>)}</div></>;
}
...
function PlayersView({ universe, teamById, onPlayer }) {
  ...
  const allPlayers=status==='Active'?universe.players.filter((player)=>player.status!=='Free Agent'):status==='Free Agents'?universe.players.filter((player)=>player.status==='Free Agent'):status==='Retired'?universe.retiredPlayers:status==='Followed'?[...universe.players,...universe.retiredPlayers].filter((player)=>(universe.followedPlayerIds??[]).includes(player.id)):[...universe.players,...universe.retiredPlayers];
  ...
  return <><div className="page-heading"><div><div className="kicker">People</div><h2>Players</h2></div><div className="page-note">Portraits persist for the entire career; rarity styling stays permanent.</div></div><section className="panel player-browser"><div className="filters six"><Filter label="Status"><select value={status} onChange={(e)=>setStatus(e.target.value)}><option>Active</option><option>Free Agents</option><option>Retired</option><option>Followed</option><option>All</option></select></Filter><Filter label="Continent"><select value={region} onChange={(e)=>setRegion(e.target.value)}><option>All</option>{unique(allPlayers.map((p)=>p.region)).map((i)=><option key={i}>{i}</option>)}</select></Filter><Filter label="Competition"><select value={competition} onChange={(e)=>setCompetition(e.target.value)}><option>All</option>{competitions.map((i)=><option key={i}>{i}</option>)}</select></Filter><Filter label="Position"><select value={position} onChange={(e)=>setPosition(e.target.value)}><option>All</option>{POSITION_ORDER.map((i)=><option key={i}>{i}</option>)}</select></Filter><Filter label="Rarity"><select value={rarity} onChange={(e)=>setRarity(e.target.value)}><option>All</option>{RARITY_ORDER.map((i)=><option key={i}>{i}</option>)}</select></Filter><Filter label="Search"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Player, team, country"/></Filter></div><SectionHeader title="Top matching players" note={`${shown.length.toLocaleString()} matching careers`}/><div className="player-card-grid">{shown.slice(0,12).map(player=><button className={`player-visual-card rarity-surface-${player.rarity.toLowerCase()}`} key={player.id} onClick={()=>onPlayer(player)}><PlayerPortrait player={player} team={teamById.get(player.teamId)} size={88}/><div className="player-card-copy"><Rarity value={player.rarity}/><strong>{player.name}</strong><span><Flag value={player.nationality}/> {player.position} · {player.teamName}</span><small>{player.stats.ppg} PPG · {player.stats.rpg} RPG · {player.stats.apg} APG</small></div><RatingPill value={player.current}/></button>)}</div></section><section className="panel section-gap"><SectionHeader title="Player database" note={`Showing first ${Math.min(400,shown.length)}`}/><PlayerTable players={shown.slice(0,400)} onPlayer={onPlayer}/></section></>;
}

function CoachesView({ universe, onCoach, onTeam }) {
  const [section,setSection]=useState('Coaches'); const [status,setStatus]=useState('Active'); const [region,setRegion]=useState('All');
...
function MarketView({ universe, tab, setTab, onPlayer, onTeam, onCoach }) {
  ...
  const [summaryCompetition,setSummaryCompetition]=useState('All');
  ...
  const tabs=['Offseason Summary','Trades','Transfers','Draft','Rights','Free Agency','Retirements','Hall of Fame','Spawn','Coaches'];
  const years=unique([
    ...
    ...universe.spawnHistory.map((item)=>item.year),...universe.coachTransactions.map((item)=>item.year),...(universe.offseasonHistory??[]).map((item)=>item.year),...(universe.legacyHistory??[]).map((item)=>item.year),
  ...
  const summaryCompetitions=unique(summaryTeams.flatMap((row)=>row.competitions?.length?row.competitions:[row.competition]).filter(Boolean)).sort((a,b)=>String(a).localeCompare(String(b)));
  const filteredSummaryTeams=summaryCompetition==='All'?summaryTeams:summaryTeams.filter((row)=>(row.competitions?.length?row.competitions:[row.competition]).includes(summaryCompetition));
  const improved=[...filteredSummaryTeams].sort((a,b)=>b.delta-a.delta||b.after-a.after).slice(0,12);
  const worsened=[...filteredSummaryTeams].sort((a,b)=>a.delta-b.delta||a.after-b.after).slice(0,12);
  const selectedLegacy=(yearFilter==='All'?(universe.legacyHistory??[])[0]:(universe.legacyHistory??[]).find((item)=>String(item.year)===String(yearFilter)))??null;
  const legacyRows=(selectedLegacy?[...(selectedLegacy.nbaInductions??[]),...(selectedLegacy.fibaInductions??[]),...(selectedLegacy.jerseyRetirements??[])]:[]).filter((item)=>rarity==='All'||item.rarity===rarity).filter((item)=>`${item.player??''} ${item.team??''} ${item.hall??''}`.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>sort==='Rarity'?rarityRank(a.rarity)-rarityRank(b.rarity)||b.score-a.score:sort==='Rating'?b.score-a.score:sort==='Name'?String(a.player).localeCompare(String(b.player)):0);
  return <><div className="page-heading"><div><div className="kicker">Offseason & Market</div><h2>Draft, trades, contracts, free agency and roster movement</h2></div><div className="page-note">The offseason now runs in inspectable stages. Basketball uses more player movement than the football build, especially through NBA trades and international transfers.</div></div>
    ...
    {tab==='Hall of Fame'&&<section className="panel"><SectionHeader title={selectedLegacy?`${selectedLegacy.year} legacy honors`:'Hall of Fame & jersey retirements'} note="Only awarded when a career ends; standards are intentionally severe"/>{sharedControls}{selectedLegacy?<><div className="metric-grid detail-metrics"><Metric label="NBA Hall of Fame" value={(selectedLegacy.nbaInductions??[]).filter((item)=>rarity==='All'||item.rarity===rarity).length} detail="Career-wide NBA achievement"/><Metric label="FIBA Hall of Fame" value={(selectedLegacy.fibaInductions??[]).filter((item)=>rarity==='All'||item.rarity===rarity).length} detail="Club + national-team achievement"/><Metric label="Jerseys retired" value={(selectedLegacy.jerseyRetirements??[]).filter((item)=>rarity==='All'||item.rarity===rarity).length} detail="Extraordinary team-specific legacy"/></div><div className="table-wrap section-gap"><table><thead><tr><th>Year</th><th>Honor</th><th>Player</th><th>Rarity</th><th>Team / Hall</th><th>Legacy score</th><th>Details</th></tr></thead><tbody>{legacyRows.map((item,index)=><tr className="clickable" key={`${item.playerId}-${item.hall??item.teamId}-${index}`} onClick={()=>onPlayer(item.playerId)}><td>{selectedLegacy.year}</td><td><strong>{item.hall?`${item.hall} Hall of Fame`:`#${item.number} retired`}</strong></td><td>{item.player}</td><td><Rarity value={item.rarity}/></td><td>{item.hall??<LinkButton onClick={()=>onTeam(item.teamId)}>{item.team}</LinkButton>}</td><td>{item.score}</td><td>{item.hall?(item.summary?`${item.summary.seasons} seasons · ${item.summary.titles} titles · ${item.summary.mvps+item.summary.finalsMvps} major MVPs · ${item.summary.firstFive} best-five · ${item.pillars??'—'} legacy pillars`:`${item.careerYears} recorded seasons`):`${item.seasons} seasons · ${item.titles} titles · ${item.mvps+Number(item.finalsMvps??0)} major MVPs · ${item.firstFive} best-five · ${item.leaders} stat leaders · ${item.pillars??'—'} pillars`}</td></tr>)}</tbody></table></div></>:<div className="empty-state">No eligible careers have ended yet. This should normally stay empty for the first several seasons.</div>}</section>}
...
function MagazineView({ universe, onPlayer }) {
  const [section,setSection]=useState('Magazine');
  const topPlayers=[...universe.players].filter((p)=>p.status==='Active').sort((a,b)=>b.current-a.current).slice(0,8);
  const draft=universe.draftHistory[0]; const nbaHall=universe.hallOfFame?.nba??[]; const fibaHall=universe.hallOfFame?.fiba??[];
  const cover=topPlayers[0];
  return <><div className="magazine-header magazine-cover"><div className="magazine-cover-copy"><div className="kicker light">Weekly world basketball magazine</div><h2>The Global Five</h2><p>Careers, power shifts, draft stories and historical context.</p></div>{cover&&<button className="magazine-cover-player" onClick={()=>onPlayer(cover)}><PlayerPortrait player={cover} size={112}/><span>WORLD No. 1</span><strong>{cover.name}</strong><small>{cover.teamName} · {cover.current} OVR</small></button>}</div><div className="tabs section-gap"><button className={section==='Magazine'?'active':''} onClick={()=>setSection('Magazine')}>Magazine</button><button className={section==='Hall of Fame'?'active':''} onClick={()=>setSection('Hall of Fame')}>Hall of Fame</button></div>{section==='Magazine'?<div className="two-column"><section className="panel"><SectionHeader title="Market headline" note="Latest offseason"/>{draft?<article className="feature-copy"><h3>{draft.ncaaPicks} NCAA players and {draft.internationalPicks} international prospects drafted</h3><p>{draft.signed} joined immediately; {draft.rightsStashed} remain connected to NBA rights while building careers elsewhere.</p></article>:<div className="empty-state">Complete a season to create the first market story.</div>}</section><section className="panel"><SectionHeader title="Players to follow" note="Open a full career"/><div className="magazine-player-list">{topPlayers.map((p)=><button className="feature-story visual-feature-story" key={p.id} onClick={()=>onPlayer(p)}><PlayerPortrait player={p} size={48}/><div><Rarity value={p.rarity}/><strong>{p.name}</strong><p>{p.teamName} · {p.stats.ppg} PPG · {p.honors.length} honors</p></div><RatingPill value={p.current}/></button>)}</div></section></div>:<div className="two-column"><HallOfFamePanel title="NBA Hall of Fame" rows={nbaHall} onPlayer={onPlayer}/><HallOfFamePanel title="FIBA Hall of Fame" rows={fibaHall} onPlayer={onPlayer}/></div>}</>;
}

function HallOfFamePanel({title,rows,onPlayer}) {return <section className="panel"><SectionHeader title={title} note={`${rows.length} inducted careers`}/><div className="rank-list">{rows.map((entry,index)=><button className="rank-row rank-button" key={`${entry.hall}-${entry.playerId}`} onClick={()=>onPlayer(entry.playerId)}><span className="rank-number">{index+1}</span><div className="grow"><strong>{entry.player}</strong><span>{entry.year} · {entry.rarity} · legacy {entry.score}{entry.summary?` · ${entry.summary.titles} titles · ${entry.summary.mvps+entry.summary.finalsMvps} major MVPs · ${entry.summary.firstFive} best-five`:''}</span></div><Rarity value={entry.rarity}/></button>)}</div>{!rows.length&&<div className="empty-state">No eligible retired career yet.</div>}</section>;}
...
function teamStrengthContext(universe, team) {
  const competitionId = team.secondaryCompetitionIds?.includes('euroleague') ? 'euroleague' : team.competitionId;
  const competition = getCompetition(competitionId);
  const peers = getCompetitionParticipants(universe, competitionId).filter((item)=>item.type!=='National');
  return { competitionId, competitionName: competition?.name ?? team.competition, peers };
}
function teamStrengthTone(universe, team, key) {
  const { peers } = teamStrengthContext(universe, team);
  const rows = peers.filter((item)=>Number.isFinite(item.strengthProfile?.[key])).sort((a,b)=>b.strengthProfile[key]-a.strengthProfile[key]);
  if (rows.length < 2) return 'average';
  const index = Math.max(0, rows.findIndex((item)=>item.id===team.id));
  const percentile = index / Math.max(1, rows.length - 1);
  if (percentile <= 0.15) return 'elite';
  if (percentile <= 0.35) return 'strong';
  if (percentile <= 0.65) return 'average';
  if (percentile <= 0.85) return 'weak';
  return 'very-weak';
}
function TeamStrengthProfile({ team, universe, before = null }) {
  const profile = team.strengthProfile ?? { overall: team.rating, perimeter: team.rawRating, interior: team.rawRating, defense: team.rawRating, rebounding: team.rawRating };
  const context = teamStrengthContext(universe, team);
  const items = [
    ['overall','Overall','roster + coach + continuity'],
    ['perimeter','Perimeter','shooting + creation'],
    ['interior','Interior','rim pressure + 2P scoring'],
    ['defense','Defense','perimeter + interior stops'],
    ['rebounding','Rebounding','board control + size'],
  ];
  return <section className="panel team-strength-panel"><SectionHeader title="Team strength profile" note={`Colors are relative to ${context.competitionName}`}/><div className="strength-profile-grid">{items.map(([key,label,detail])=>{const value=profile[key]??'—';const prior=before?.[key];const delta=Number.isFinite(prior)&&Number.isFinite(value)?Number((value-prior).toFixed(1)):null;return <div className={`strength-profile-card strength-${teamStrengthTone(universe,team,key)}`} key={key}><span>{label}</span><strong>{value}</strong><small>{detail}{delta!=null?` · ${delta>=0?'+':''}${delta}`:''}</small></div>;})}</div></section>;
}
function OffseasonProfileDelta({ row }) {
  const before=row.profileBefore??{};const after=row.profileAfter??{};const fields=[['perimeter','Perimeter'],['interior','Interior'],['defense','Defense'],['rebounding','Rebounding']];
  if(!Object.keys(after).length)return null;
  return <div className="movement-profile-deltas">{fields.map(([key,label])=>{const delta=Number.isFinite(before[key])&&Number.isFinite(after[key])?Number((after[key]-before[key]).toFixed(1)):null;return <span key={key}><b>{label}</b> {before[key]??'—'} → {after[key]??'—'} {delta!=null&&<em className={delta>0?'positive':delta<0?'negative':''}>{delta>=0?'+':''}{delta}</em>}</span>;})}</div>;
}

function TeamPage({ team, universe, onBack, onPlayer, onCoach, onCompetition }) {
  const [tab,setTab]=useState('Overview'); const roster=team.rosterIds.map((id)=>universe.players.find((p)=>p.id===id)).filter(Boolean).sort((a,b)=>POSITION_ORDER.indexOf(a.position)-POSITION_ORDER.indexOf(b.position)||b.current-a.current); const coach=[...universe.coaches,...universe.retiredCoaches].find((item)=>item.id===team.coachId); const owner=[...universe.owners,...universe.formerOwners].find((item)=>item.id===team.ownerId); const locals=roster.filter((p)=>p.nationality===team.country||(team.type==='NBA'&&['USA','Canada'].includes(p.nationality))).length; const stars=[...roster].sort((a,b)=>b.current-a.current).slice(0,5);
  return <><DetailHeader onBack={onBack}><div className="entity-hero team-entity-hero" style={{'--entity-color':team.color}}><TeamMark team={team} size={96} className="hero-badge"/><div className="entity-hero-copy"><div className="kicker"><Flag value={team.country}/> {team.country} · {team.region}</div><h2>{team.name}</h2><p className="linked-competitions"><LinkButton onClick={()=>onCompetition(team.competitionId)}>{team.competition}</LinkButton>{team.secondaryCompetitions.map((name,index)=><React.Fragment key={`${name}-${index}`}><span> · </span><LinkButton onClick={()=>onCompetition(team.secondaryCompetitionIds[index])}>{name}</LinkButton></React.Fragment>)}</p><div className="entity-chip-row"><span>{team.wins}–{team.losses}</span><span>{team.honors.length} titles</span><span>{team.continuity??0}% continuity</span></div></div><RatingPill value={team.rating} label="WORLD"/></div></DetailHeader><div className="metric-grid detail-metrics"><Metric label="World rating" value={team.rating} detail={`Raw roster ${team.rawRating}`}/><Metric label="Record" value={`${team.wins}–${team.losses}`} detail={team.competition}/><Metric label="Titles" value={team.honors.length} detail="Permanent cabinet"/><Metric label="Local players" value={`${locals}/${roster.length}`} detail={`Minimum ${team.localMinimum}`}/></div><DetailTabs tabs={['Overview','Seasons','Honors','Retired jerseys','Leadership','Transfers']} tab={tab} setTab={setTab}/>
    {tab==='Overview'&&<><TeamStrengthProfile team={team} universe={universe}/><section className="panel section-gap"><SectionHeader title="Core players" note="The five highest-rated players on the current roster"/><div className="lineup-card-grid">{stars.map(player=><button className={`lineup-player rarity-surface-${player.rarity.toLowerCase()}`} key={player.id} onClick={()=>onPlayer(player)}><PlayerPortrait player={player} team={team} size={82}/><span className="lineup-pos">{player.position}</span><strong>{player.name}</strong><Rarity value={player.rarity}/><small>{player.stats.ppg} PPG · {player.stats.rpg} RPG</small><RatingPill value={player.current}/></button>)}</div></section><div className="two-column detail-columns section-gap"><section className="panel"><SectionHeader title="Current roster" note={`${roster.length} players`}/><PlayerTable players={roster} onPlayer={onPlayer} compact/></section><section className="panel team-identity-panel"><SectionHeader title="Current identity" note="Leadership affects performance and recruitment"/>{coach&&<button className="identity-person-card" onClick={()=>onCoach(coach.id)}><CoachPortrait coach={coach} team={team} size={64}/><div><small>HEAD COACH</small><strong>{coach.name}</strong><span>{coach.style} · {coach.current} OVR</span></div></button>}<InfoLine label="President / owner" value={owner?`${owner.name} · ${owner.profile}`:'Unassigned'}/><InfoLine label="Owner rarity" value={owner?.rarity??'—'}/><InfoLine label="Prestige" value={team.prestige}/><InfoLine label="Continuity" value={`${team.continuity??0}%`}/><InfoLine label="Tier" value={team.tier===2?'Second tier':'Top tier'}/><InfoLine label="NCAA alumni" value={roster.filter((p)=>p.originRoute==='NCAA').length}/></section></div></>}
    ...
    {tab==='Honors'&&<TeamHonorsPanel honors={team.honors}/>} {tab==='Retired jerseys'&&<RetiredJerseysPanel team={team} onPlayer={onPlayer}/>} {tab==='Leadership'&&<div className="two-column"><section className="panel"><SectionHeader title="Head coach" note="Current appointment"/>{coach?<><div className="identity-person-card static"><CoachPortrait coach={coach} team={team} size={88}/><div><Rarity value={coach.rarity}/><strong>{coach.name}</strong><span>{coach.style} · {coach.current} OVR</span></div></div><InfoLine label="Offense / defense" value={`${coach.offense} / ${coach.defense}`}/><InfoLine label="Development" value={coach.development}/><InfoLine label="Contract through" value={coach.contractEnd}/></>:<div className="empty-state">Vacant</div>}</section><section className="panel"><SectionHeader title="President / owner" note="Mandate and bonuses"/>{owner?<><InfoLine label="Name" value={owner.name}/><InfoLine label="Profile" value={owner.profile}/><InfoLine label="Rarity" value={owner.rarity}/><InfoLine label="Mandate" value={`${owner.startYear}–${owner.endYear}`}/><InfoLine label="Recruitment / stability" value={`${owner.recruitment} / ${owner.stability}`}/></>:<div className="empty-state">Vacant</div>}<h3 className="subheading">Past mandates</h3>{team.leadershipHistory.slice(0,10).map((item)=><div className="info-line" key={item.ownerId}><span>{item.startYear}–{item.endYear}</span><strong>{item.name} · {item.rarity}</strong></div>)}</section></div>} {tab==='Transfers'&&<TeamMovementHistory team={team} universe={universe} onPlayer={onPlayer} onCoach={onCoach}/>}</>;
}

function PlayerPage({ player, universe, onBack, onTeam, onCompetition, onToggleFollow }) {
  const [tab,setTab]=useState('Overview'); const currentTeam=universe.teams.find((t)=>t.id===player.teamId); const rightsTeam=player.rightsTeamId?universe.teams.find((t)=>t.id===player.rightsTeamId):null; const collegeYearsShown=player.originRoute==='NCAA'?(player.teamType==='NCAA'?4:Math.min(4,Math.max(1,(player.yearsInNCAA??0)+1))):0; const developmentCurve=player.originRoute==='NCAA'?[...(player.ncaaCurve??[]).slice(0,collegeYearsShown),...(player.proCurve??[])]:player.careerCurve; const developmentIndex=player.originRoute==='NCAA'?(player.teamType==='NCAA'?Math.min(collegeYearsShown-1,player.yearsInNCAA??0):collegeYearsShown+Math.max(0,(player.proYears??1)-1)):player.careerYear; const followed=(universe.followedPlayerIds??[]).includes(player.id);
  return <><DetailHeader onBack={onBack}><div className={`entity-hero player-entity-hero rarity-surface-${player.rarity.toLowerCase()}`} style={{'--entity-color':currentTeam?.color||'#315d9b'}}><PlayerPortrait player={player} team={currentTeam} size={118} className="hero-portrait"/><div className="entity-hero-copy"><div className="kicker"><Flag value={player.nationality}/> {player.nationality} · {player.age} · {player.status}</div><h2>{player.name}{player.realIdentity&&<span className="historical-identity-badge">Historical pool</span>}</h2><p>{currentTeam?<LinkButton onClick={()=>onTeam(currentTeam.id)}>{player.teamName}</LinkButton>:player.teamName} · #{player.jerseyNumber??'—'} · {player.position} · {player.role}</p><div className="entity-chip-row"><Rarity value={player.rarity}/><span>Base {player.base}</span><span>{player.height} cm · {player.body}</span></div></div><RatingPill value={player.current}/><button className={`follow-player ${followed?'active':''}`} onClick={()=>onToggleFollow?.(player.id)} title={followed?'Remove bookmark':'Bookmark player'}>{followed?'★':'☆'}<span>{followed?'Following':'Follow'}</span></button></div></DetailHeader><div className="metric-grid detail-metrics"><Metric label="Rarity" value={player.rarity} detail="Permanent"/><Metric label="Base" value={player.base} detail="Permanent ability"/><Metric label="Current" value={player.current} detail={`Career year ${player.careerYear+1}`}/><Metric label="Honors" value={player.honors.length} detail={`${player.internationalHistory?.length??0} international appearances`}/></div><DetailTabs tabs={['Overview','Career','International','Honors','Timeline']} tab={tab} setTab={setTab}/>
    {tab==='Overview'&&<><div className="two-column detail-columns"><section className="panel"><SectionHeader title="Career identity" note={player.careerProfile}/><InfoLine label="Position" value={player.position}/><InfoLine label="Jersey" value={player.jerseyNumber!=null?`#${player.jerseyNumber}`:"—"}/><InfoLine label="Height / body" value={`${player.height} cm · ${player.body}`}/><InfoLine label="Role" value={player.role}/><InfoLine label="Origin route" value={player.originRoute}/>{player.realIdentity&&<InfoLine label="Historical identity" value="Real-name legend pool"/>}<InfoLine label="Career length" value={`${player.careerYears} years`}/><InfoLine label="Current multiplier" value={player.developmentMultiplier??developmentCurve[developmentIndex]??'—'}/>{player.draft&&<><InfoLine label="NBA Draft" value={`${player.draft.year} · Pick ${player.draft.pick} · ${player.draft.team}`}/><InfoLine label="Current NBA rights" value={rightsTeam?.name??(player.nbaJoinedYear?'Activated':'None')}/></>}<InfoLine label="Contract" value={player.contract?`${player.contract.team} · ${player.contract.salaryTier} · through ${player.contract.endYear}`:'No active contract'}/></section><section className="panel production-panel"><SectionHeader title="Current production" note={`${player.stats.games} games`}/><div className="stat-block-grid"><div><strong>{player.stats.ppg}</strong><span>PTS</span></div><div><strong>{player.stats.rpg}</strong><span>REB</span></div><div><strong>{player.stats.apg}</strong><span>AST</span></div><div><strong>{player.stats.spg}</strong><span>STL</span></div><div><strong>{player.stats.bpg}</strong><span>BLK</span></div></div><InfoLine label="Rebound split" value={`${player.stats.orpg} ORB · ${player.stats.drpg} DRB`}/><InfoLine label="Shooting" value={`${player.stats.fg}% FG · ${player.stats.three}% 3P · ${player.stats.ft}% FT`}/></section></div><section className="panel"><SectionHeader title="Career progression" note={player.originRoute==='NCAA'?'College years are capped at 0.89; professional years use the adult curve':'Base and rarity never change'}/><div className="career-year-scroll"><div className="career-year-strip">{developmentCurve.map((value,index)=>{const displayed=Math.round(player.base*value);return <div key={index} className={`career-year-card ${index===developmentIndex?'current-year':''}`}><small>Y{index+1}</small><strong>{displayed}</strong><span>{Number(value).toFixed(2)}</span></div>;})}</div></div></section></>}
    {tab==='Career'&&<section className="panel"><SectionHeader title="Detailed annual breakdown" note="Team, contract, production, ability and honors"/><div className="table-wrap"><table><thead><tr><th>Year</th><th>Age</th><th>#</th><th>Team</th><th>Competition</th><th>Contract</th><th>GP</th><th>PTS</th><th>REB</th><th>AST</th><th>STL</th><th>BLK</th><th>OVR</th><th>Honors</th></tr></thead><tbody>{[...player.history].reverse().map((season,index)=><tr key={`${season.year}-${season.teamId}-${index}`}><td><strong>{season.year}</strong></td><td>{season.age}</td><td>{season.jerseyNumber!=null?`#${season.jerseyNumber}`:"—"}</td><td>{season.teamId?<LinkButton onClick={()=>onTeam(season.teamId)}>{season.team}</LinkButton>:season.team}</td><td>{season.competitionId!=='free-agency'?<LinkButton onClick={()=>onCompetition(season.competitionId)}>{season.competition}</LinkButton>:season.competition}</td><td>{season.contract?`${season.contract.salaryTier} · ${season.contract.endYear}`:'—'}</td><td>{season.games}</td><td>{season.ppg}</td><td>{season.rpg}</td><td>{season.apg}</td><td>{season.spg}</td><td>{season.bpg}</td><td><strong>{season.current}</strong></td><td>{formatHonorList(season.honors)}</td></tr>)}</tbody></table></div></section>}
...
function CompetitionPage({ competition, universe, onBack, onPlayer, onTeam }) {
  ...
  return <><DetailHeader onBack={onBack}><div className="entity-hero competition-entity-hero"><CompetitionEmblem competition={competition} size={92}/><div className="entity-hero-copy"><div className="kicker"><Flag value={competition.country || competition.region}/> {competition.region} · {competition.country || 'International'} · {competition.kind}</div><h2>{competition.name}</h2><p>{participants.length || latest?.standings?.length || 0} teams · {isCompetitionActive(competition,universe.year)?'active this year':competition.frequency?`next edition follows a ${competition.frequency}-year cycle`:'domestic competition'}</p><div className="entity-chip-row">{latest&&<><span>{latest.year} champion: {latest.champion}</span><span>MVP: {latest.mvp.name}</span></>}</div></div></div></DetailHeader><div className="metric-grid detail-metrics"><Metric label="Teams" value={participants.length||latest?.standings?.length||0} detail="Current / latest edition"/><Metric label="Recorded seasons" value={histories.length} detail="Permanent archive"/><Metric label="Latest champion" value={latest?.champion??'—'} detail={latest?String(latest.year):'No completed edition'}/><Metric label="Latest MVP" value={latest?.mvp.name??'—'} detail={latest?.mvp.team??'No award yet'}/></div><DetailTabs tabs={['Overview','Bracket','Seasons','Awards','Rankings','Teams']} tab={tab} setTab={setTab}/>
    {tab==='Overview'&&<div className="two-column detail-columns"><section className="panel"><SectionHeader title="Current standings" note="Regular season / seeding"/><div className="table-wrap"><table><thead><tr><th>#</th><th>Team</th><th>W</th><th>L</th><th>Win %</th><th>Rating</th></tr></thead><tbody>{standings.slice(0,32).map((entry,index)=><tr className="clickable" key={entry.team.id} onClick={()=>onTeam(entry.team.id)}><td>{index+1}</td><td><span className="table-identity"><TeamMark team={entry.team} size={32}/><strong><Flag value={entry.team.country}/> {entry.team.name}</strong></span></td><td>{entry.wins}</td><td>{entry.losses}</td><td>{pct(entry.pct)}</td><td><strong>{entry.team.rating}</strong></td></tr>)}</tbody></table></div></section><section className="panel"><SectionHeader title="Current leaders" note="Projected until year review"/><div className="leader-cards"><LeaderLine label="MVP race" person={leaders.mvp} onPlayer={onPlayer}/><LeaderLine label="Points" person={leaders.points} suffix={leaders.points ? `${leaders.points.stats.ppg} PPG` : null} onPlayer={onPlayer}/><LeaderLine label="Rebounds" person={leaders.rebounds} suffix={leaders.rebounds ? `${leaders.rebounds.stats.rpg} RPG` : null} onPlayer={onPlayer}/><LeaderLine label="Assists" person={leaders.assists} suffix={leaders.assists ? `${leaders.assists.stats.apg} APG` : null} onPlayer={onPlayer}/></div></section></div>}
    ...
    {tab==='Awards'&&<section className="panel"><SectionHeader title="Awards by season" note="MVP, statistical leaders and the best five by position"/><div className="table-wrap"><table><thead><tr><th>Year</th><th>MVP</th><th>Finals / playoff MVP</th><th>Points</th><th>Rebounds</th><th>Assists</th><th>Steals</th><th>Blocks</th><th>PG</th><th>SG</th><th>SF</th><th>PF</th><th>C</th></tr></thead><tbody>{histories.map((season)=><tr key={`awards-${season.year}`}><td><strong>{season.year}</strong></td><td>{season.mvp?<LinkButton onClick={()=>onPlayer(season.mvp.id)}>{season.mvp.name}</LinkButton>:'—'}</td><td>{season.finalsMvp?<LinkButton onClick={()=>onPlayer(season.finalsMvp.id)}>{season.finalsMvp.name}</LinkButton>:'—'}</td>{['points','rebounds','assists','steals','blocks'].map((key)=><td key={key}>{season.leaders?.[key]?<><LinkButton onClick={()=>onPlayer(season.leaders[key].id)}>{season.leaders[key].name}</LinkButton><small className="table-subvalue"> {season.leaders[key].value}</small></>:'—'}</td>)}{POSITION_ORDER.map((position)=><td key={position}>{season.allLeagueFive?.[position]?<LinkButton onClick={()=>onPlayer(season.allLeagueFive[position].id)}>{season.allLeagueFive[position].name}</LinkButton>:'—'}</td>)}</tr>)}</tbody></table></div>{!histories.length&&<div className="empty-state">Awards appear after the first completed edition.</div>}</section>}
    ...
    {tab==='Teams'&&<div className="card-grid four team-card-grid-modern">{participants.sort((a,b)=>b.rating-a.rating).map((team)=><button className="team-card modern-team-card" key={team.id} onClick={()=>onTeam(team.id)} style={{'--team-color':team.color}}><div className="team-card-glow"/><div className="team-card-top"><TeamMark team={team} size={56}/><RatingPill value={team.rating}/></div><h3>{team.name}</h3><p><Flag value={team.country}/> {team.country}</p></button>)}</div>}</>;
}
...
function PlayerTable({ players,onPlayer,draft=false,highlight=null,compact=false }) {const value=(player)=>highlight?(highlight in player.stats?player.stats[highlight]:player[highlight]):null;return <div className="table-wrap"><table className={compact?'compact-table':''}><thead><tr><th>Player</th><th>Age</th><th>#</th><th>Team</th><th>Pos</th>{!compact&&<><th>Body</th><th>Role</th></>}<th>Rarity</th><th>Base</th><th>Current</th>{draft&&<th>Potential</th>}{highlight&&<th>Selected stat</th>}<th>PTS</th><th>REB</th><th>AST</th><th>Contract</th></tr></thead><tbody>{players.map((player)=><tr className="clickable" key={player.id} onClick={()=>onPlayer(player)}><td><span className="table-person"><PlayerPortrait player={player} size={36}/><span><strong>{player.name}</strong><small><Flag value={player.nationality}/> {player.nationality}</small></span></span></td><td>{player.age}</td><td>{player.jerseyNumber!=null?player.jerseyNumber:'—'}</td><td>{player.teamName}</td><td><strong>{player.position}</strong></td>{!compact&&<><td>{player.height} cm · {player.body}</td><td>{player.role}</td></>}<td><Rarity value={player.rarity}/></td><td>{player.base}</td><td><strong className="ovr-number">{player.current}</strong></td>{draft&&<td>{player.potential}</td>}{highlight&&<td><strong>{value(player)}</strong></td>}<td>{player.stats.ppg}</td><td>{player.stats.rpg}</td><td>{player.stats.apg}</td><td>{player.contract?`${player.contract.salaryTier} · ${player.contract.endYear}`:'—'}</td></tr>)}</tbody></table></div>;}
...
function RetiredJerseysPanel({team,onPlayer}) {const rows=[...(team.retiredJerseys??[])].sort((a,b)=>Number(b.year)-Number(a.year)||Number(a.number)-Number(b.number));return <section className="panel"><SectionHeader title="Retired jerseys" note="Reserved for extraordinary team-specific careers"/><div className="retired-jersey-grid">{rows.map((entry)=><button className="retired-jersey-card" key={`${entry.number}-${entry.playerId}`} onClick={()=>onPlayer(entry.playerId)}><span>#{entry.number}</span><strong>{entry.player}</strong><small>{entry.year} · {entry.seasons} seasons</small><p>{entry.titles} titles · {entry.mvps+Number(entry.finalsMvps??0)} major MVPs · {entry.firstFive} best-five · {entry.leaders??0} leaders</p></button>)}</div>{!rows.length&&<div className="empty-state">No number has reached the club's retirement standard yet.</div>}</section>;}
function TeamMovementHistory({team,universe,onPlayer,onCoach}) {
  const summaries=(universe.offseasonHistory??[]).map((offseason)=>({year:offseason.year,row:(offseason.teams??[]).find((item)=>item.teamId===team.id)})).filter((item)=>item.row);
  return <><section className="panel"><SectionHeader title="Offseason movement" note="Who came in, who left, and whether the coach changed"/><div className="team-movement-history">{summaries.map(({year,row})=><article className="team-movement-year" key={year}><div className="movement-year-head"><div><strong>{year}</strong><span>{row.before} → {row.after} ({row.delta>=0?'+':''}{row.delta})</span></div><small>{row.departures??0} out · {row.arrivals??0} in · target {row.turnoverTarget??'—'}</small></div><OffseasonProfileDelta row={row}/><div className="movement-columns"><div><h4>Players out</h4>{(row.playersOut??[]).length?(row.playersOut??[]).map((player)=><button key={player.playerId} onClick={()=>onPlayer(player.playerId)}><span><Rarity value={player.rarity}/></span><strong>{player.player}</strong><small>{player.position} · Base {player.base} · OVR {player.current}</small></button>):<em>No player departures</em>}</div><div><h4>Players in</h4>{(row.playersIn??[]).length?(row.playersIn??[]).map((player)=><button key={player.playerId} onClick={()=>onPlayer(player.playerId)}><span><Rarity value={player.rarity}/></span><strong>{player.player}</strong><small>{player.position} · Base {player.base} · OVR {player.current}</small></button>):<em>No player arrivals</em>}</div></div>{(row.coachOut||row.coachIn)&&<div className="coach-movement"><strong>Coach</strong>{row.coachOut?<button onClick={()=>onCoach(row.coachOut.coachId)}>Out: {row.coachOut.coach} · {row.coachOut.rarity} · Base {row.coachOut.base}</button>:<span>No coach out</span>}<span>→</span>{row.coachIn?<button onClick={()=>onCoach(row.coachIn.coachId)}>In: {row.coachIn.coach} · {row.coachIn.rarity} · Base {row.coachIn.base}</button>:<span>No coach in</span>}</div>}</article>)}</div>{!summaries.length&&<div className="empty-state">Complete an offseason to create the first team movement report.</div>}</section><TransactionPanel transactions={team.transactions} onPlayer={onPlayer}/></>;
}
