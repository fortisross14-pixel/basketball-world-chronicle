...
function normalizeUniverseForUi(universe) {
  ...
  universe.version ??= 7;
  if (Number(universe.version) >= 9 && Number(universe.version) < 9.2) universe = repairMarketBalance(universe);
...
export default function App() {
  ...
  const continueGame = async (slot) => {
    ...
    try {
      ...
      setDetail(null);
      goToRoute('world', true);
      setSaveStatus((record.version ?? loadedUniverse.version ?? 7) < 9.2 ? 'Older save loaded — v0.9.2 legacy fields are migrated where possible; start a new universe for a completely clean jersey and coach history.' : 'Save loaded');
    } catch (error) {
