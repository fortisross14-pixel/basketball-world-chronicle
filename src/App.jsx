...
export default function App() {
  ...
  const createNewGame = async (slot) => {
    ...
    setLoadingSaves(true);
    try {
      const next = createUniverse((Date.now() + slot * 7919) % 2147483647);
  ...
  const openCoach = (coachOrId) => setDetail({ type: 'coach', id: typeof coachOrId === 'object' ? coachOrId.id : coachOrId });
  ...
  return <div className="app-shell">
    ...
    <nav className="main-nav">{NAV.map((item) => <button key={item} className={!detail && view === item ? 'active' : ''} onClick={() => navigate(item)}>{item}</button>)}</nav>
    <main>
      ...
      {detail ? <DetailRouter detail={detail} universe={universe} teamById={teamById} playerById={playerById} coachById={coachById} onBack={() => setDetail(null)} onTeam={openTeam} onPlayer={openPlayer} onCoach={openCoach} onCompetition={openCompetition} /> : <>
        ...
        {view === 'Market' && <MarketView universe={universe} tab={marketTab} setTab={setMarketTab} onPlayer={openPlayer} onTeam={openTeam} onCoach={openCoach} />}
...
function TournamentsView({ universe, onCompetition }) {
  ...
  const regionTeams = universe.teams.filter((team) => team.region === region && team.type !== 'National');
  const countries = unique(regionTeams.map((team) => team.country));
  const topCompetitions = COMPETITIONS.filter((competition) => competitionInRegion(competition, region) && (
    competition.kind === 'continental' || competition.kind === 'international' || competition.featured ||
    ['NBA','NBA G League','NCAA Division I','NCAA Tournament'].includes(competition.name)
  )).sort((a,b) => Number(Boolean(b.featured)) - Number(Boolean(a.featured)) || b.level - a.level);
  const countryCompetitions = country ? COMPETITIONS.filter((competition) => competition.country === country || competition.country?.split(' / ').includes(country) || competition.country?.includes(country)) : [];
  return <><div className="page-heading"><div><div className="kicker">Tournament universe</div><h2>Continent → top competitions → country</h2></div><div className="page-note">Every competition opens into standings, seasons, brackets, rankings and participating teams.</div></div>
    <div className="tabs">{regions.map((item) => <button key={item} className={region === item ? 'active' : ''} onClick={() => { setRegion(item); setCountry(null); }}><Flag value={item}/> {item}</button>)}</div>
    {!country && <><SectionHeader title={`${region} top competitions`} note="Continental, international and headline competitions"/><CompetitionCards competitions={topCompetitions} universe={universe} onCompetition={onCompetition}/>{region !== 'World' && <><SectionHeader title="Countries" note="Select a country to see all domestic competitions"/><div className="country-grid">{countries.map((item) => <button key={item} onClick={() => setCountry(item)}><strong><Flag value={item}/> {item}</strong><span>{universe.teams.filter((team) => team.country === item && team.type !== 'National').length} teams</span></button>)}</div></>}</>}
    {country && <><button className="back-button country-back" onClick={() => setCountry(null)}>← Back to {region}</button><div className="page-heading compact-heading"><div><div className="kicker">{region}</div><h2>{country}</h2></div></div><CompetitionCards competitions={countryCompetitions} universe={universe} onCompetition={onCompetition}/></>}
  </>;
}
